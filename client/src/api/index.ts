...
const HttpManager = {
  ...
  getCollectionPlaylistOfUser: (userId) => get(`/collection/detaillist?userId=${userId}`), 
  // 添加收藏的歌曲 type: 0 代表歌曲， 1 代表歌单
  setCollection: ({userId,type,songId}) => post(`collection/add`,{userId,type,songId}),
  setlistcollection: ({userId,type,songListId}) => post(`collection/add`,{userId,type,songListId}),

  deleteCollection: (userId, songId) => deletes(`collection/delete?userId=${userId}&&songId=${songId}`),
  deletesonglistCollection : (userId, songListId) => deletes(`collection/deletelist?userId=${userId}&&songListId=${songListId}`),

  isCollection: ({userId, type, songId}) => post(`collection/status`, {userId, type, songId}),
  issonglistollection: ({userId, type, songListId}) => post(`collection/statuslist`, {userId, type, songListId}),
  ...
  // 返回指定歌单ID的歌曲
  getPlaylistOfId: (id) => get(`playlist/detail?id=${id}`),
  // 返回指定歌手ID的歌曲

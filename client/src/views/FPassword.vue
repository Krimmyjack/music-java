  ...
  methods: {
    async sendVerificationCode() {
      try {
         ...
         console.log(response.data); //如果请求成功，将后端返回的数据打印到控制台，并使用 this.$message 显示成功消息。
        this.$message({
          message: response.data,
          type: 'success'
        });
/*       
在后端，需要有相应的处理逻辑来接收这个请求。后端代码应该包含以下功能：
接收请求：监听到 GET /user/sendVerificationCode 请求，并提取电子邮件参数。
生成验证码：生成一个验证码，并将其发送到指定的电子邮件地址。
响应结果：向前端返回成功或失败的消息。
*/

...
export default defineComponent({
  ...
  setup() {
    ...
    async function handleLoginIn() {
      let canRun = true;
      (proxy.$refs["signInForm"] as any).validate((valid) => { //通过 proxy.$refs["signInForm"] 获取登录表单的引用，并调用 validate 方法进行表单的验证
        if (!valid) return (canRun = false);
      });
      if (!canRun) return; //如果表单未通过验证，函数将早期返回，停止执行后续的逻辑
      ...
      try {
        ...
        (proxy as any).$message({ //调用 proxy.$message 显示登录状态的消息。例如，成功或失败的提示
        ...
        if (result.success) {
          ...
          proxy.$store.commit("setToken", true); //这些 commit 操作的目的是将用户在登录时获取到的信息存储到 Vuex 中
          changeIndex(NavName.Home);
          routerManager(RouterName.Home, { path: RouterName.Home }); //调用 changeIndex(NavName.Home) 和 routerManager 更新当前界面并进行路由跳转到主页
        }
      } catch (error) {
        console.error(error);
      }

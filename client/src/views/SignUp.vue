...
export default defineComponent({
  ...
  setup() {
    ...
    const registerForm = reactive({  //定义了一个 registerForm 变量，使用 reactive 函数来创建一个响应式对象，以存储用户在注册表单中输入的信息
      username: "",
      password: "",
      sex: "",
      phoneNum: "",
    ...
    async function goBackRegist() {
       routerManager(RouterName.SignIn, { path: RouterName.SignIn }); //处理表单提交
    }

    async function handleSignUp() {
      let canRun = true;
      (proxy.$refs["signUpForm"] as any).validate((valid) => { //验证表单，确认输入数据有效
      ...
      try {
        ...
        (proxy as any).$message({ //将 registerForm 对象中的用户输入的数据传递给 HttpManager.SignUp 方法，该方法负责与后端进行通信，发送注册请求
          message: result.message,
          type: result.type,
        });

        if (result.success) { //根据返回的结果（如注册成功或失败）来显示相应的消息，并根据成功与否进行路由的跳转
          changeIndex(NavName.SignIn); //NavName.SignIn 是一个常量，表示登录页面的名称。
          routerManager(RouterName.SignIn, { path: RouterName.SignIn });  //RouterName.SignIn 同样是一个常量，表示路由名称，注册成功返回登陆页面
        }
      } catch (error) {
        console.error(error);
      }

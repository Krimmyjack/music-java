<template>
  <div class="play-bar" :class="{ show: !toggle }">
    ...
    <!--播放进度-->
    <el-slider class="progress" v-model="nowTime" @change="changeTime" :format-tooltip="formatTooltip" size="small"></el-slider>
...
export default defineComponent({
  ...
  methods: {
    ...
    changeTime() {
      const timeInSeconds = this.duration * (this.nowTime * 0.01);
      this.$store.commit("setChangeTime", this.duration * (this.nowTime * 0.01));
      // 格式化时间并显示
      this.displayTime = this.formatTime(timeInSeconds);
    },
    formatTime(seconds) {
    ...
    formatTooltip(value) {
      // 将百分比转换为实际时间（秒）
      const timeInSeconds = this.duration * (value * 0.01);
      // 格式化时间
    ...
    next() {
      ...
      if (this.playState === Icon.LUANXU) {
        ...
        if (this.currentPlayIndex < this.currentPlayList.length - 1) {
          // 如果当前播放歌曲不是最后一首，则播放下一首
          this.$store.commit("setIsPlay", false);
          this.$store.commit("setCurrentPlayIndex", this.currentPlayIndex + 1);
          this.toPlay(this.currentPlayList[this.currentPlayIndex].url);
        } else {
          // 如果当前播放的是最后一首，则跳转到第一首
          this.$store.commit("setIsPlay", false);
